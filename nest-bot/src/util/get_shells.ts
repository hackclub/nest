...
export default async function get_shells(shell: string, shellPath?: string): Promise<string[]> {
    return new Promise((resolve, reject) => {
        try {
            const shellFile = fs.readFileSync(shellPath || "/etc/shells", "utf8").split("\n").filter(line => !/^\s*#/.test(line))
            ...
            const shells: string[] = uniqueShells
                .map(shell => shellFile.find(line => line.includes(shell) && line.includes("/bin/")) || shellFile.find(line => line.includes(shell))) as string[];
