...
(async () => {
  ...
  for (const user of users) {
    const pkReq = await fetch(
      `https://identity.hackclub.app/api/v3/core/users/?username=${encodeURIComponent(user.tilde_username!)}`,
      {
        headers: {
    ...
    if (!pkReq.ok) {
      console.error(
        `Failed to get user ${user.email} in Authentik (HTTP code ${pkReq.status})`,
      );
      continue;
    }
