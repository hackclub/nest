...
async def resolve(ts: str, resolver: str, client: AsyncWebClient, stale: bool = False):
    ...
    if not resolving_user:
        await send_heartbeat(
            f"User {resolver} attempted to resolve ticket with ts {ts} but isn't in the database. https://hackclub.slack.com/archives/{env.slack_help_channel}/p{ts.replace('.', '')}",
    ...
    if not allowed:
        await send_heartbeat(
            f"User {resolver} attempted to resolve ticket with ts {ts} without permission. https://hackclub.slack.com/archives/{env.slack_help_channel}/p{ts.replace('.', '')}",
            messages=[f"Ticket TS: {ts}", f"Resolver ID: {resolver}"],
        )
        return
    ...
    if not tkt:
        await send_heartbeat(
            f"Failed to resolve ticket with ts {ts} by {resolver}. Ticket not found. https://hackclub.slack.com/archives/{env.slack_help_channel}/p{ts.replace('.', '')}",
            messages=[f"Ticket TS: {ts}", f"Resolver ID: {resolver}"],
        )
        return
